...
AUTH_PASSWORD = os.environ.get("AUTH_PASSWORD", "").encode("utf-8")
if not AUTH_PASSWORD:
    print("ERROR: AUTH_PASSWORD environment variable must be set before execution.")
    print("Usage: AUTH_PASSWORD=<your_password> python SampleNetworkClient.py")
    exit(1)
...
class SimpleNetworkClient :
    ...
    def updateInfTemp(self, frame) :
        ...
        if self.infToken is None : #not yet authenticated
            self.infToken = self.authenticate(self.infPort, AUTH_PASSWORD)
    ...
    def updateIncTemp(self, frame) :
        ...
        if self.incToken is None : #not yet authenticated
            self.incToken = self.authenticate(self.incPort, AUTH_PASSWORD)
